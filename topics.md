...
本ドキュメントは、カリキュラムには具体的には含まれないが、知っておいてほしいトピックをまとめたものです。特定のステップに含めることができなかった話題はここにまとめておきます。
...
`git rebase -i` を利用するとコミットの前後を入れ替えられるので
...
コマンドを直接実行しなくても git の公式サイトに便利な GUI ツール集があるので色々試してみると良いでしょう。
...
用途としては、ある機能の実装をした場合、ブラウザから挙動を確認できますが、モデルに対してメソッドを追加した場合など、画面で１から動作を見るよりも、rails console で手軽に確認したほうが効率がよいことがよくあります。
...
pry を利用すると、アプリケーションの実行中に好きな場所で対話的な実行環境(REPL)を開いて
現在の変数の状態や実行結果を得ることができるようになります。

公式ドキュメント: https://github.com/pry/pry/wiki

#### Railsにおけるインストール方法

Gemfile に以下の一文を追加します。（開発時にだけ使用するため、:development, :test 環境でのみ導入します）
...
`bundle install` を実行すると使えるようになります。

#### 使用方法

好きなコードに対して、次のように `binding.pry` を記述します。
...
アプリケーションを実行した時点で、 `binding.pry` を記述した部分に処理が到達すると、そこで REPL が立ち上がるので、その時点で使用できる変数を見たり、メソッドを実行することができます。
