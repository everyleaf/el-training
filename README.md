...
- 以下のコマンドでDockerをビルドしてアプリを立ち上げましょう
    ...
    - 以下のように表示されれば正常にアプリが立ち上がっています
      ```sh
      api-1     | => Booting Puma
      api-1     | => Rails 7.0.8.1 application starting in development 
      api-1     | => Run `bin/rails server --help` for more startup options
      api-1     | Puma starting in single mode...
      api-1     | * Puma version: 5.6.8 (ruby 3.1.2-p20) ("Birdie's Version")
      api-1     | *  Min threads: 5
      api-1     | *  Max threads: 5
...
- 作成したアプリをGitHub上に作成したブランチにpushしましょう
  - pushする前にgitignoreを設定したほうがおすすめ：[GitHub - simonwhitaker/gibo: Easy access to gitignore boilerplates](https://github.com/simonwhitaker/gibo)
    ```sh
