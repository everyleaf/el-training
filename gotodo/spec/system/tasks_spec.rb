  ...
  describe '#new' do
    context '正常系' do
      ...
      it '新規登録したタスクが表示されること' do
        expect(page).to have_current_path task_path Task.last.id.to_s
    ...
    context '異常系' do
      let(:new_task) { FactoryBot.create(:task, title: nil, detail: nil) }
      before do
        visit new_task_path
        fill_in 'タスク名', with: new_task.title
        fill_in '詳細', with: new_task.detail
      ...
      it 'Flashメッセージが表示されること' do
        ...
        expect(page).to have_selector('#alert', text: 'タスクの作成に失敗しました。')
      end
    end
  end
